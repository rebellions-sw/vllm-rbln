...
def set_lora_mask(
    ...
    for input, lora_id in zip(inputs, prompt_mapping):
        ...
        mask = torch.zeros(
            input_len, max_loras * max_lora_rank, dtype=lora_dtype
        )
        if (
            lora_id != 0
            and (lora_index := id_to_index.index(lora_id)) is not None
        ):
            mask[
                :,
                lora_index * max_lora_rank : lora_index * max_lora_rank
                + max_lora_rank,
            ] = 1
        masks.append(mask)
...
def test_lm_head_logits_processor(
    ...
    for i in range(NUM_RANDOM_SEEDS):
        ...
        set_sampler_indices_padded(
            prompt_mapping, id_to_index, len(inputs), False, max_loras
        )
        ...
        for input_, lora_id in zip(inputs, prompt_mapping):
            ...
            linear.weight[vocab_size : vocab_size + embeddings_tensor_len] = (
                embeddings_tensor
            )
