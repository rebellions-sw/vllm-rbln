...
from tests.torch_compile.v1.attention.utils import (
    BatchSpec, _Backend, create_common_attn_metadata,
    create_standard_kv_cache_spec, create_vllm_config, get_attention_backend)

BACKENDS_TO_TEST = [_Backend.FLASH_ATTN_VLLM_V1]
...
def run_attention_backend(backend: _Backend, kv_cache_spec: FullAttentionSpec,
    ...
    extra_attn_metadata_args = {
        "num_tokens": np.zeros(query.shape[0]),
        "positions": torch.zeros(query.shape[0] + 1)
    }
    builder = builder_cls(kv_cache_spec, layer_names, vllm_config, device)
    attn_metadata = builder.build(common_prefix_len=0,
    ...
    output = impl.forward(mock_layer,
                          ...
                          value.view(value.shape[0], value.shape[1], -1),
                          kv_cache,
                          attn_metadata,
                          output=output)
...
def test_backend_correctness(
    ...
    for i in range(batch_size):
        ...
        q = torch.randn(batch_size,
                        ...
                        dtype=dtype,
                        device=device)
        k_full = torch.randn(batch_size,
                             s_len,
                             num_kv_heads,
                             head_size,
                             dtype=dtype,
                             device=device)
        v_full = torch.randn(batch_size,
                             s_len,
                             num_kv_heads,
                             head_size,
                             dtype=dtype,
        ...
        q_sdpa_in = q.transpose(1, 2)
        k_sdpa_in = k_full.transpose(1, 2)
        v_sdpa_in = v_full.transpose(1, 2)
        ...
        all_sdpa_outputs.append(sdpa_out_i.transpose(1, 2))

        # Inputs for vLLM backends are just the new tokens
        all_q_vllm.append(q)
        all_k_vllm.append(k_full[0, context_len:])
        all_v_vllm.append(v_full[0, context_len:])

        # Contextual K/V data used to populate the paged cache
        k_contexts.append(k_full[0, :context_len])
        v_contexts.append(v_full[0, :context_len])

    query_vllm = torch.cat(all_q_vllm, dim=1)
    key_vllm = torch.cat(all_k_vllm, dim=1)
    value_vllm = torch.cat(all_v_vllm, dim=1)
    sdpa_output = torch.cat(all_sdpa_outputs, dim=1)
    ...
    with set_current_vllm_config(vllm_config):
        for backend_name in BACKENDS_TO_TEST:
            ...
            backend_output = run_attention_backend(
                backend_name, kv_cache_spec, ["placeholder"], vllm_config,
                device, common_attn_metadata, query_vllm, key_vllm, value_vllm,
                kv_cache_for_backend)

            # Check shape and dtype consistency
            sdpa_output_reshaped = sdpa_output.reshape(sdpa_output.shape[0],
                                                       sdpa_output.shape[1],
                                                       -1)
            assert backend_output.shape == sdpa_output_reshaped.shape, (
                f"[{backend_name}] shape {backend_output.reshape.shape} != "
                f"SDPA shape {sdpa_output_reshaped.shape}")
            ...
            # TODO(RBLN): Compare rbln flash attention output with sdpa output
            # Check numerical similarity
            # rtol = 1e-2
            # atol = 5e-3

            # max_diff = torch.max(
            #     torch.abs(backend_output - sdpa_output_reshaped)).item()
            # max_rel_diff = torch.max(
            #     torch.abs(backend_output - sdpa_output_reshaped) /
            #     torch.abs(sdpa_output_reshaped)).item()
            # all_close = torch.allclose(backend_output,
            #                         sdpa_output_reshaped,
            #                         rtol=rtol,
            #                         atol=atol)
