...
def set_lora_mask(inputs: list[torch.Tensor], prompt_mapping: list[int],
    ...
    for input, lora_id in zip(inputs, prompt_mapping):
        ...
        mask = torch.zeros(input_len,
                           ...
                           dtype=lora_dtype)
        if lora_id != 0 and (lora_index :=
                             id_to_index.index(lora_id)) is not None:
            mask[:, lora_index * max_lora_rank:lora_index * max_lora_rank +
                 max_lora_rank] = 1
        masks.append(mask)
...
def test_embeddings(dist_init, num_loras, device, vocab_size, stage) -> None:
    ...
    for i in range(NUM_RANDOM_SEEDS):
        ...
        lora_dict, _ = populate_loras(id_to_index,
...
@pytest.mark.parametrize("fully_shard",
                         [False])  # WithSharedLoRA version is not implemented.
@pytest.mark.parametrize("device", DEVICES)
@pytest.mark.parametrize("stage", STAGES)
...
def test_column_parallel_packed(dist_init, num_loras, repeats, fully_shard,
    ...
    for i in range(NUM_RANDOM_SEEDS):
        ...
        for input_, lora_id in zip(inputs, prompt_mapping):
            ...
            result = rbln_compile(lora_column_parallel_packed_ref)(input_,
                                                                   linear,
                                                                   subloras)
            expected_results.append(result)
        expected_result = torch.cat(expected_results)
...
@pytest.mark.skip("This test has become invalid as a result of "
