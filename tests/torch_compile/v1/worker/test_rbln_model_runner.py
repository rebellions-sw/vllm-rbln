...
def initialize_kv_cache(runner: RBLNModelRunner):
    """
    Only perform necessary steps in RBLNModelRunner.initialize_kv_cache()
    """
...
def model_runner():
    ...
    with set_current_vllm_config(vllm_config):
        ...
        vllm_config.compilation_config.static_forward_context[
            "layer.0"] = Attention(num_heads, head_size, 0.1, prefix="layer.0")
        runner = RBLNModelRunner(vllm_config, DEVICE)
...
@pytest.mark.skip(reason="TODO: needs dist_init")
...
def test_init_kv_cache_without_kv_sharing(monkeypatch):
    torch.set_default_dtype(torch.float16)
    layer_0 = "model.layers.0.self_attn.attn"
    layer_1 = "model.layers.1.self_attn.attn"
    vllm_config = get_vllm_config()
    with set_current_vllm_config(vllm_config):
        ...
        runner.compile_context = CompileContext(use_weight_sharing=True)
