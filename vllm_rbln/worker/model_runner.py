...
class RBLNModelRunner(ModelRunnerBase[ModelInputForRebelWithSamplingMetadata]):
    ...
    def prepare_model_input(
        ...
        if is_prompt:
            logger.debug("[RBLN] Prefill step - requests: %s", [seq_group_metadata.request_id for seq_group_metadata in seq_group_metadata_list])
        else:
            logger.debug("[RBLN] Decode step - requests: %s", [seq_group_metadata.request_id for seq_group_metadata in seq_group_metadata_list])
        ...
        with set_forward_context(model_input.attn_metadata, self.vllm_config,
            ...
            if envs.RBLN_METRICS:
                ...
                if model_input.is_prompt:
                    total_tokens = sum(model_input.query_lens) if model_input.query_lens else 0
                    self.performance_tracker.record_prefill(execution_time, total_tokens)
                else:
