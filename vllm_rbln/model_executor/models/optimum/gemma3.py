...
class RBLNGemma3MultiModalProcessor(Gemma3MultiModalProcessor):
    ...
    def apply(self, *args, **kwargs):
        ...
        prompt_ids, prompt = self._pad_for_gemma3(
            output["prompt_token_ids"], output["prompt"]
        )
...
class RBLNOptimumGemma3ForConditionalGeneration(
    ...
    def pad_local_table_items(
        ...
        pad_value = next(
            (i for i in range(self.decoder_batch_size) if i not in used_ids), 0
        )
        ...
        local_block_table_id[:request_nums] = torch.tensor(
            sliding_window_table_ids, dtype=torch.int16
        ).unsqueeze(1)

        padded_cache_lengths_tensor = torch.zeros(
            padded_batch_size, 1, dtype=position_id_dtype
        )
        padded_cache_lengths_tensor[:request_nums] = torch.tensor(
            padded_cache_lengths, dtype=position_id_dtype
        ).unsqueeze(1)

        attention_mask_dtype = (
            attention_masks[0].dtype if attention_masks else torch.bool
        )
        attention_mask = torch.zeros(
            padded_batch_size, seq_len, dtype=attention_mask_dtype
        )
        if attention_masks:
    ...
    def select_local_block_table_value(
        ...
        if is_prompt:
            attention_mask = (
                (input_ids != self.pad_token_id).to(torch.int64).squeeze(0)
            )
        else:
        ...
        if is_prompt:
            ...
            result = cast(
                Tuple[list[int], list[int], list[torch.Tensor]], result
            )
