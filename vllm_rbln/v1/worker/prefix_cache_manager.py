...
class RBLNPrefixBlockQueue:
    ...
    def popleft(self) -> int:
        if len(self.blocks) == 0:
            raise RuntimeError(
                "No free outer blocks available: prefix cache block management exhausted."
            )
...
class PrefixKVCacheManager:
    ...
    def __init__(self, vllm_config, num_ob):
        self.req_to_outer_blocks: dict[str, list[int]] = {}
        self.inner_to_request_id: dict[int,
                                       str] = {}  # inner block의 outdated 체크 목적
        self.inner_to_outer_block: dict[int, int] = {}

        self.ob_size = vllm_config.additional_config.attn_block_size
        self.ib_size = vllm_config.cache_config.block_size
