...
class CacheSearchManager:
    ...
    def find_cached_blocks(
        ...
        best_match = self._try_match_request(
            cached_blocks, skip_blocks, mapping_manager
        )
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            if best_match.has_cache_hit:
                ...
                for i, (ob, cached_length) in enumerate(
                    zip(best_match.cached_outer_blocks, best_match.cached_lengths)
                ):
...
class RBLNPrefixKVCacheManager:
    ...
    def set_cached_blocks(
        self, cached_blocks: list[int], allocated_outer_blocks: list[int]
    ) -> None:
        """
        Mark the blocks associated with the request as cached.
        """
        self._mapping_manager.set_cached_blocks(
            cached_blocks, allocated_outer_blocks, self._config.block_ratio
        )
