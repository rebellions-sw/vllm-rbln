...
# RBLN batched attention, flash attention decode optimized version for large batch
@torch.library.custom_op(
    "rbln_custom_ops::flash_causal_batch_attention_naive_decode",
    mutates_args=["kv_cache"])
def flash_causal_batch_attention_naive_decode_impl(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    kv_cache: torch.Tensor,
    ...
    # seq_idx[B,1] - original sequence index
    # make dyn_size_for_partitions from original seq_idx
    cs = seq_idx.repeat(1, num_partition)
    pidx = torch.arange(num_partition, dtype=torch.int32)
    dyn_size_for_partitions = torch.clamp(cs - pidx * partition_size, 0,
                                          partition_size).to(torch.int16)

    outputs = []
    for b in range(batch_size):
        # Calculate the starting position (number of tokens already in cache)
        cache_start_pos = int(dyn_size_for_partitions[b].sum().item())
        total_seq_len = cache_start_pos + seq_len  # seq_len is 1 for decode
...
class RBLNFlashAttentionMetadataBuilder(
    ...
    def build(
        ...
        # seq_lens_tensor(otherwise)      - [B, P], have dynamic size for each partition
        attn_metadata = RBLNFlashAttentionMetadata(
            ...
            if not self.is_batch_attention_opt or is_prefills[0] else seq_idx.to(self.device),
...
class RBLNFlashAttentionImpl(AttentionImpl[RBLNFlashAttentionMetadata]):
    ...
    def __init__(
        ...
        logger.info("RBLN flash attention impl CAUSAL = %s, BATCH_ATTN_OPT = %s",
            self.is_causal, self.is_batch_attention_opt)
