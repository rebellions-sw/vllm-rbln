...
class RBLNFlashAttentionImpl(AttentionImpl[RBLNFlashAttentionMetadata]):
    ...
    def forward(
        ...
        if self.sliding_window is not None:
            ...
            if q_len == 1:
                attn_output = sliding_window_attention_decode(  # noqa: E501
                    query,
                    key,
                    value,
                    kv_cache,
                ...
                attn_output = sliding_window_attention_prefill(  # noqa: E501
                    query,
                    key,
                    value,
                    kv_cache,
        ...
        # actually non-flash paged attention DOES NOT use slot_mapping
            ...
            if self.is_normal:
                ...
                if envs.VLLM_RBLN_COMPILE_MODEL:
                    if envs.VLLM_RBLN_KERNEL_MODE == "torch_triton":
                        causal_attention_prefill = (
                            torch.ops.rbln_triton_ops.causal_attention_prefill
                        )
                        causal_attention_decode = (
                            torch.ops.rbln_triton_ops.causal_attention_decode)
                    elif envs.VLLM_RBLN_KERNEL_MODE == "triton":
                ...
                if q_len == 1:
                    attn_output = causal_attention_decode(  # noqa: E501
                        query,
                        key,
                        value,
                        kv_cache,
                    ...
                    attn_output = causal_attention_prefill(  # noqa: E501
                        query,
                        key,
                        value,
                        kv_cache,
                ...
                if envs.VLLM_RBLN_COMPILE_MODEL:
                    ...
                    flash_causal_attention_decode = (
                        flash_causal_attention_decode_impl)
                ...
                if q_len == 1:
                    attn_output = flash_causal_attention_decode(  # noqa: E501
                        query,
                        key,
                        value,
                        kv_cache,
                    ...
                    attn_output = flash_causal_attention_prefill(  # noqa: E501
                        query,
                        key,
                        value,
                        kv_cache,
            ...
            if self.is_normal:
                ...
                if envs.VLLM_RBLN_COMPILE_MODEL:
                    if envs.VLLM_RBLN_KERNEL_MODE == "torch_triton":
                        attention_prefill = (
                            torch.ops.rbln_triton_ops.attention_prefill
                        )
                        attention_decode = (
                            torch.ops.rbln_triton_ops.attention_decode)
                    elif envs.VLLM_RBLN_KERNEL_MODE == "triton":
                        raise ValueError("RBLN custom kernel is not supported for attention naive prefill/decode")
                    else:
                        raise ValueError(f"Invalid VLLM_RBLN_KERNEL_MODE: "
                ...
                if q_len == 1:
                    attn_output = attention_decode(  # noqa: E501
                        query,
                        key,
                        value,
                    ...
                    attn_output = attention_prefill(  # noqa: E501
                        query,
                        key,
                        value,
                        attn_metadata.attn_masks,
                ...
                if envs.VLLM_RBLN_COMPILE_MODEL:
                    if envs.VLLM_RBLN_KERNEL_MODE == "torch_triton":
                        flash_attention_prefill = (
                            torch.ops.rbln_triton_ops.flash_attention_prefill
                        )
                        flash_attention_decode = (
                            torch.ops.rbln_triton_ops.flash_attention_decode)
                    elif envs.VLLM_RBLN_KERNEL_MODE == "triton":
                        flash_attention_prefill = (
                            torch.ops.rbln_custom_ops.flash_attention_prefill
                        )
                        flash_attention_decode = (
                            torch.ops.rbln_custom_ops.flash_attention_decode)
                    else:
                        raise ValueError(f"Invalid VLLM_RBLN_KERNEL_MODE: "
                                         f"{envs.VLLM_RBLN_KERNEL_MODE}")
                else:
                    flash_attention_prefill = (
                        flash_attention_prefill_impl)
                    flash_attention_decode = (
                        flash_attention_decode_impl)

                if q_len == 1:
                    attn_output = flash_attention_decode(  # noqa: E501
                        query,
                        key,
                        value,
                        kv_cache,
                    ...
                    attn_output = flash_attention_prefill(  # noqa: E501
                        query,
                        key,
                        value,
                        kv_cache,
